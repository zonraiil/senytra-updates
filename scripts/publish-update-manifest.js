...
const EXPECTED_PUBLIC_KEY_SHA256 =
  'dfd884c49270e1163a9800527f077198cf3a8f71e76ef0275a95d692c411baaa'
...
function releaseNotes(file) {
  if (!file || !fs.existsSync(file)) {
    return ['Performans, güvenlik ve kararlılık iyileştirmeleri.']
  }
  ...
  return notes.length
    ? notes
    : ['Performans, güvenlik ve kararlılık iyileştirmeleri.']
}
...
function main() {
  ...
  if (!/^\d+\.\d+\.\d+$/.test(version)) {
    throw new Error('Sürüm 2.0.0 biçiminde olmalıdır')
  }

  if (!/^https:\/\//i.test(download) || /[\[\]()]/.test(download)) {
    throw new Error('İndirme adresi normal bir HTTPS adresi olmalıdır')
  }

  if (!fs.existsSync(installer)) {
    throw new Error(`Kurucu bulunamadı: ${installer}`)
  }

  if (!fs.existsSync(privateKeyPath)) {
    throw new Error(`İmzalama anahtarı bulunamadı: ${privateKeyPath}`)
  }
  ...
  if (fingerprint !== EXPECTED_PUBLIC_KEY_SHA256) {
    throw new Error(
      'İmzalama anahtarı bu Senytra sürümünün güncelleme anahtarıyla eşleşmiyor'
    )
  }
  ...
  if (
    ...
    decoded.product !== 'Senytra'
  ) {
    throw new Error(
      'Oluşturulan manifest son doğrulamadan geçmedi'
    )
  }

  console.log(`Senytra güncelleme manifesti hazır: v${version}`)
